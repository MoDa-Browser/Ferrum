...
use moda_render::{DOMNode, LayoutEngine, NodeType, Rect};
use moda_sandbox::{NamespaceConfig, Sandbox};
use moda_security::{Capability, CapabilityToken, PermissionManager, PermissionPolicy};
...
fn main() {
    ...
    println!(
        "Sandbox supported: {}",
        platform_info.platform().supports_sandbox()
    );
    println!();
    ...
    println!("--------------------");
    let namespace_config = NamespaceConfig::new().with_pid(true).with_network(true);

    let _sandbox = Sandbox::new().with_namespace(namespace_config);
    ...
    if let Some(bounds) = layout_engine.get_element_bounds("root") {
        println!(
            "Root element bounds: x={}, y={}, width={}, height={}",
    ...
    println!(
        "Created DOM node: {} with bounds ({}, {}, {}, {})",
        div_node.tag_name,
        div_node.bounds.x,
        div_node.bounds.y,
        div_node.bounds.width,
        div_node.bounds.height
    );
    println!();
